...
#if defined (HAS_SYS_STATFS_H) || defined (HAS_SYS_STATVFS_H) || defined (HAS_SYS_VFS_H) || \
    defined (HAS_SYS_MOUNT_H)
unsigned long husky_GetDiskFreeSpace(const char * path)
{
...
#else  /* if defined (HAS_SYS_STATFS_H) || defined (HAS_SYS_STATVFS_H) || defined
         (HAS_SYS_VFS_H) || defined (HAS_SYS_MOUNT_H) */
unsigned long husky_GetDiskFreeSpace(const char * path)
{
...
#endif /* if defined (HAS_SYS_STATFS_H) || defined (HAS_SYS_STATVFS_H) || defined
          (HAS_SYS_VFS_H) || defined (HAS_SYS_MOUNT_H) */


#elif defined (__MSC__) || defined (__DJGPP__) /* alternate variand for DJGPP with DOS Fn's
                                                  error check */
unsigned long husky_GetDiskFreeSpace(const char * path)
{
    int diskno;
    ...
    {
        return 0;                      /* illegal diskno */
    }

    if(_dos_getdiskfree(diskno, &df))
    {
...
#endif /* if defined (__DPMI__) && !defined (__DJGPP__) */
} /* husky_GetDiskFreeSpace */
