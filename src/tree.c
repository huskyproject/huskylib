...
static char * tree_srch_real(tree ** ppr_tree, int (* pfi_compare)(char *, char *), char * pc_user)
{
    ...
    if(*ppr_tree)
    {
        ...
        if(i_comp > 0)
        {
            EXIT(tree_srch_real(&(**ppr_tree).tree_r, pfi_compare, pc_user))
        }

        if(i_comp < 0)
        {
            EXIT(tree_srch_real(&(**ppr_tree).tree_l, pfi_compare, pc_user))
        }
...
static int sprout(tree ** ppr,
                  char * pc_data,
                  int * pi_balance,
                  int (* pfi_compare)(char *, char *),
                  int (* pfi_delete)(char *),
                  char need_balance)
{
    tree * p1, * p2;
    ...
    if(!*ppr)
    {
        PRMSG("grounded. adding new node, setting h=true") * ppr = (tree *)smalloc(sizeof(tree));
        (*ppr)->tree_l = NULL;
        (*ppr)->tree_r = NULL;
        (*ppr)->tree_b = 0;
        (*ppr)->tree_p = pc_data;
    ...
    if(cmp < 0)
    {
        PRMSG("LESS. sprouting left.") nRet = sprout(&(*ppr)->tree_l,
                                                     pc_data,
                                                     pi_balance,
                                                     pfi_compare,
                                                     pfi_delete,
        ...
        if(*pi_balance)    /* left branch has grown longer */
        {
            ...
            switch((*ppr)->tree_b)
            {
                case 1: /* right branch WAS longer; balance is ok now */
                    PRMSG("LESS: case 1.. balnce restored implicitly")(*ppr)->tree_b = 0;
                    *pi_balance = FALSE;
                    break;

                case 0: /* balance WAS okay; now left branch longer */
                    PRMSG("LESS: case 0.. balnce bad but still ok")(*ppr)->tree_b = -1;
                    break;

                case -1:
                    /* left branch was already too long. rebalnce */
                    PRMSG("LESS: case -1: rebalancing") p1 = (*ppr)->tree_l;

                    if(p1->tree_b == -1) /* LL */
                    {
                        PRMSG("LESS: single LL")(*ppr)->tree_l = p1->tree_r;
                        p1->tree_r     = *ppr;
                        (*ppr)->tree_b = 0;
                        *ppr           = p1;
                    }
                    ...
                    {
                        PRMSG("LESS: double LR") p2 = p1->tree_r;
                        p1->tree_r     = p2->tree_l;
                        p2->tree_l     = p1;
    ...
    if(cmp > 0)
    {
        PRMSG("MORE: sprouting to the right") nRet = sprout(&(*ppr)->tree_r,
                                                            pc_data,
                                                            pi_balance,
                                                            pfi_compare,
                                                            pfi_delete,
        ...
        if(*pi_balance)    /* right branch has grown longer */
        {
            ...
            {
                case -1:
                    PRMSG("MORE: balance was off, fixed implicitly")(*ppr)->tree_b = 0;
                    *pi_balance = FALSE;
                    break;

                case 0:
                    PRMSG("MORE: balance was okay, now off but ok")(*ppr)->tree_b = 1;
                    break;

                case 1:
                    PRMSG("MORE: balance was off, need to rebalance") p1 = (*ppr)->tree_r;

                    if(p1->tree_b == 1) /* RR */
                    {
                        PRMSG("MORE: single RR")(*ppr)->tree_r = p1->tree_l;
                        p1->tree_l     = *ppr;
                        (*ppr)->tree_b = 0;
                        *ppr           = p1;
                    }
                    ...
                    {
                        PRMSG("MORE: double RL") p2 = p1->tree_l;
                        p1->tree_l     = p2->tree_r;
                        p2->tree_r     = p1;
     ...
     */
    PRMSG("I found it!  Replacing data value") * pi_balance = FALSE;
...
static int tree_add_real(tree ** ppr_tree,
                         int (* pfi_compare)(char *, char *),
                         char * pc_user,
                         int (* pfi_delete)(char *),
                         char need_balance)
{
    ...
    ENTER("tree_add") nRet = sprout(ppr_tree,
                                    pc_user,
                                    &i_balance,
                                    pfi_compare,
                                    pfi_delete,
...
int tree_add(tree ** ppr_tree, int (* pfi_compare)(char *,
                                                   char *), char * pc_user, int (* pfi_delete)(
                 char *))
{
...
static void balanceR(tree ** ppr_p, int * pi_balance)
{
    ...
    ENTER("balanceR") PRMSG("right branch has shrunk")

    switch((*ppr_p)->tree_b)
    {
        case 1:
            PRMSG("was imbalanced, fixed implicitly")(*ppr_p)->tree_b = 0;
            break;

        case 0:
            PRMSG("was okay, is now one off")(*ppr_p)->tree_b = -1;
            *pi_balance = FALSE;
            break;

        case -1:
            PRMSG("was already off, this is too much") p1 = (*ppr_p)->tree_l;
            b1 = p1->tree_b;

            if(b1 <= 0)
            {
                PRMSG("single LL")(*ppr_p)->tree_l = p1->tree_r;
                p1->tree_r = *ppr_p;

                if(b1 == 0)
                {
                    PRMSG("b1 == 0")(*ppr_p)->tree_b = -1;
                    p1->tree_b  = 1;
                    *pi_balance = FALSE;
                }
                else
                {
                    PRMSG("b1 != 0")(*ppr_p)->tree_b = 0;
                    p1->tree_b = 0;
                }
...
static void balanceL(tree ** ppr_p, int * pi_balance)
{
    ...
    ENTER("balanceL") PRMSG("left branch has shrunk")
    ...
    switch((*ppr_p)->tree_b)
    {
        ...
        case 1:
            PRMSG("was already off, this is too much") p1 = (*ppr_p)->tree_r;
            b1 = p1->tree_b;

            if(b1 >= 0)
            {
                PRMSG("single RR")(*ppr_p)->tree_r = p1->tree_l;
                p1->tree_l = *ppr_p;
                   ...
                   int * pi_uar_called)
{
    ...
    if(*ppr_p == NULL)
    {
        PRMSG("key not in tree") EXIT(FALSE)
    }
    ...
    if(i_comp > 0)
    {
        PRMSG("too high - scan left") i_ret = _delete(&(*ppr_p)->tree_l,
                                                      pfi_compare,
                                                      pc_user,
                                                      pfi_uar,
                                                      pi_balance,
    ...
    {
        PRMSG("too low - scan right") i_ret = _delete(&(*ppr_p)->tree_r,
                                                      pfi_compare,
                                                      pc_user,
                                                      pfi_uar,
                                                      pi_balance,
    ...
    {
        PRMSG("equal") pr_q = *ppr_p;

        if(pr_q->tree_r == NULL)
        {
            PRMSG("right subtree null") * ppr_p = pr_q->tree_l;
            *pi_balance = TRUE;
        }
        else if(pr_q->tree_l == NULL)
        {
            PRMSG("right subtree non-null, left subtree null") * ppr_p = pr_q->tree_r;
            *pi_balance = TRUE;
        }
        else
        {
            PRMSG("neither subtree null") del(&pr_q->tree_l,
                                              pi_balance,
                                              &pr_q,
                                              pfi_uar,
                                              pi_uar_called);
...
static int tree_delete_real(tree ** ppr_p,
                            int (* pfi_compare)(char *, char *),
                            char * pc_user,
                            int (* pfi_uar)(char *))
{
    ...
    ENTER("tree_delete");
    EXIT(_delete(ppr_p, pfi_compare, pc_user, pfi_uar, &i_balance, &i_uar_called))
}

int tree_delete(tree ** ppr_p,
                int (* pfi_compare)(char *, char *),
                char * pc_user,
                int (* pfi_uar)(char *))
{
