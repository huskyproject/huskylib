...
int cmpfnames(const char * file1, const char * file2)
{
    ...
    if(!GetFullPathName(buf, sizeof(path1), path1, &p))
    {
        return 1;
    }
    ...
    if(!GetFullPathName(buf, sizeof(path2), path2, &p))
    {
        return 1;
    }
...
int cmpfnames(const char * file1, const char * file2)
{
    ...
    /* DosQueryPathInfo declaration in os2emx.h:
       ULONG DosQueryPathInfo (PCSZ pszPathName, ULONG ulInfoLevel, PVOID pInfoBuffer, ULONG
          ulInfoLength);
       (PCSZ defined as const char* or unsigned const char* for different conditions)
     */
    if(DosQueryPathInfo((PCSZ)file1, FIL_QUERYFULLNAME, path1,
                        sizeof(path1)) ||
