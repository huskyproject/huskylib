...
int move_file(const char * from, const char * to, const int force_rewrite)
{
    ...
    if(!rc)
    {
#endif /* if !(defined (USE_SYSTEM_COPY) && (defined (__NT__) || defined (__OS2__))) */
        return 0;
    }
...
#endif /* if !(defined (USE_SYSTEM_COPY) && (defined (__NT__) || defined (OS2))) */
...
int lockFile(const char * lockfile, int advisoryLock)
{
    ...
    if(advisoryLock > 0)
    {
        while(advisoryLock > 0)
        {
            ...
            if(fh < 0)
            {
                /*                fprintf(stderr,"cannot open/create lock file: %s wait %d
            ...
            else
            {
                if(write(fh, " ", 1) != 1)
                {
                    /*                    fprintf(stderr,"can't write to lock file! wait %d
                       seconds\n", advisoryLock);*/
                    close(fh);
                    fh = -1;
                    advisoryLock--;
                ...
                {
                    /*                    fprintf(stderr,"lock file used by another process! %d
                       seconds\n", advisoryLock);*/
                    close(fh);
                    fh = -1;
                    advisoryLock--;
...
char * GetDirnameFromPathname(const char * pathname)
{
    ...
    return rp;
} /* GetDirnameFromPathname */

void fillCmdStatement(char * cmd,
                      const char * call,
                      const char * archiv,
                      const char * file,
                      const char * path)
