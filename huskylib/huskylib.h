...
#define fc_GetDiskFreeSpace(p) husky_GetDiskFreeSpace(p)  /*for compatibility with old code; to
                                                             be removed */
/* ioutil.c */
...
/*DOC
   ...
   FZ:     fgetsUntil0 reads chars into the buffer until eof(f) || n-1 are read || a \0 is
      encountered.
 */
...
/* will be moved to huskylib */
HUSKYEXT void fillCmdStatement(char * cmd,
                               const char * call,
                               const char * archive,
                               const char * file,
                               const char * path);
 ...
 */
HUSKYEXT char * GenVersionStr(const char * programname,
                              unsigned major,
                              unsigned minor,
                              unsigned patchlevel,
                              unsigned branch,
 ...
 */
HUSKYEXT int CheckHuskylibVersion(int need_major,
                                  int need_minor,
                                  int need_patch,
